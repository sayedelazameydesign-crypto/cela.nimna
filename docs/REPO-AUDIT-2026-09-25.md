...
### decision-S301 — كاش الرؤية: codec JSON موقّع، لا pickle إطلاقًا (2026-09-25)

**المرحلة 0 — التتبع الإمبيريكي (حسم الاختيار حسب جدول القاعدة):**
```
المستدعي الوحيد للكتابة في الكود كله:
  nimna/core/agent.py:820 → _vc.set(raw, _w, _h,
      {"b64": str, "mime": str, "caption": str, "hash": str})   # dict نصي حرفي
عقد القراءة (agent.py:738): isinstance(dict) and "b64" in it
الكاتب محروس بـtry/except (فشل التسلسل → fallback الذاكرة — تدهور رشيق)
مطابقة grep الأولى على "cache.set(" كانت داخل docstring — الكاتب الحقيقي وُجد بالتتبع اليدوي
⟹ كل القيم JSON-safe → القرار: JSON مباشر بلا غلاف قيم، وحذف pickle من الجذر
```

**التصميم المنفَّذ (بانحرافين موثقين عن مواصفة الغلاف المهجن، كليهما نحو الأبسط):**
1. **لا `_RestrictedUnpickler` أصلًا** — عبارة «لا مخلفات pickle إطلاقًا» في صف جدول القاعدة نُفِّذت حرفيًا: القراءة الصارمة `J1`+JSON فقط، وكل ما عداها (بما فيها مخلفات pickle) = **miss لا يُفكّ تسلسلها بشيء**. أقوى أمنيًا (لا يُحلَّل pickle خبيث حتى لرفضه) والتكلفة صفرًا عمليًا (TTL ≤ 600 ثانية).
2. البادئة `J1` أبُقيت على الكتابة فقط — تمهيدًا لأي توسعة أنواع مستقبلية دون تغيير المفتاح (dedup سليم يبقى).
3. عدّاد hits لا يزيد إلا على فك ناجح (المخلفات القديمة تعد misses بصدق).

**الاختبارات (4):** roundtrip ببادئة J1 · بايتات GLOBAL خبيثة حرفيًا (`cposix\nsystem\n...`) → miss **بلا تنفيذ** · مخلفات pickle سليمة → miss مقصود · تكامل كامل عبر VisionCache بعميل Redis مزيّف + حقن مخلفات قديمة → miss مع عدّادات سليمة.

**حادثتان موثقتان أثناء التنفيذ (الدرس نفسه تكرر):** تعديلان متوازيان على ملف واحد في دفعة واحدة ضاع الثاني مرتين (import json ثم تعديل get) — أُنتجت حالة مختلطة (كاتب J1/قارئ pickle قديم) اكتشفها اختبار الـstub فورًا. القاعدة المقننة: **تعديل واحد لكل ملف في الدفعة الواحدة، والتحقق بالقراءة بعد كل apply**.

**التحقق الختامي:** ruff أخضر (S301 خرج من ignore بتعليق «أُصلح فعلاً») · bandit B301 = 0 · `grep pickle nimna/` = صفر كود (3 أسطر توثيق فقط) · **pytest 517/517** (513 + 4) · suite: 7/0 بلا راية و9/0 معها — سلوك مطابق.

**لوحة الدين بعد S301:**
```
✅ مُمحى: F811(6) · F841(17) · S324(6) · S301(1) · 502 fix آلي · بوابة Ruff حاجبة
🔴 المرحلة 1 المتبقية: S104(4 — قرار الربط) · S110/S112(52 — logging) · S603/S607(16 — مسارات)
🟡 المرحلة 2: mypy 109 · ruff format + إعادة E501
