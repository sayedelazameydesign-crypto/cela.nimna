...
| G21 Arabic governance paths | `pytest -q tests/test_arabic_evaluation.py` | offline, mock provider, loopback socket | PASS / MOCKED for model quality | 8 governance paths with Arabic input; Unicode tool names and globs; Arabic audit trail. Coverage: `docs/arabic-evaluation.md` |
| G22 MCP security invariants | `pytest -q tests/test_mcp_security_invariants.py` | offline + subprocess on a temp copy | PASS | forced `confirm` at the execution point, `run_id` on every notification, no non-`build_agent` path; each tripwire mutation-proved |

## Interpretation

- `PASS` = the named check executed and passed in the named environment.
- `G21` is scoped to governance paths, **not** model quality: every case runs on the mock provider, so it says nothing about how a real model reasons in Arabic. See `docs/arabic-evaluation.md`.
